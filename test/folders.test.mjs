...
test('the taxonomy is exactly the spec\'s 5-folder, 38-slug assignment (folder taxonomy/nav spec section 1.1)', () => {
  const byFolder = {};
  ...
  assert.deepEqual(byFolder, {
    ...
    text: ['remove-duplicate-lines', 'sort-lines', 'text-case-converter', 'word-frequency-counter', 'word-character-counter'].sort(),
    developer: ['base64-encode-decode', 'hash-generator', 'heic-to-jpg-png', 'html-entity-encode-decode', 'regex-tester', 'sql-formatter', 'url-encode-decode', 'uuid-generator', 'unix-timestamp-converter'].sort(),
  });
});
